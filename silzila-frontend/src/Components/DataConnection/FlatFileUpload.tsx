...
const FlatFileUpload = ({
  ...
  // }, []);
  // const handleDialogClose = useCallback((val:boolean) => {
  //   setSheetChooseDialogShow(val);
  // }, []);
  const decodedToken: any = jwtDecode(token);
  const access = decodedToken.access;

  const MAX_FILE_SIZE_MB = access === "community" ? 50 : 100; // File size limit /*Business User limit -100MB Community User - 50MB*/

  
  
  
  const SheetDialog = ({sheetChooseDialogShow, setSheetChooseDialogShow }: {
          ...
          {sheetLoading ? (
            ...
            >
              <DialogTitle
                ...
                sx={{ fontSize: fontSize.large ,fontWeight:'bold',
                  paddingInline:0,}}
              >
                Select Sheet
              </DialogTitle>
          ...
          {!sheetLoading ? (
            ...
            >
              <HoverButton
                ...
                onClick={() => {setSheetChooseDialogShow(false)
  ...
  const handleFileSelect = (file: File | undefined) => {
    if (file) {
      ...
      if (file.size > maxFileSize) {
        setErrorDialogMessage(`File size exceeds the maximum limit of ${MAX_FILE_SIZE_MB} MB.`);
        setErrorDialogOpen(true);
        setSelectedFile(undefined);
        // setSelectedSheetName("");
        return;
    ...
    reader.onload = async (e: any) => {
      ...
      if (selectedFileType === "excel") return;
        ...
        var result: any = await FetchData({
          requestType: "withData",
          method: "POST",
          url: `file-upload?filetype=${selectedFileType}`,
          data: formData,
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
            ...
            {/* <div style={{ height: "60px" }}>
              {selectedFileType === "excel" && (
                <div className="excelContainer">
                  <label className="sheetNameLabel" style={{fontSize:fontSize.medium,color:palette.primary.contrastText}}>ENTER SHEET NAME:</label>
                  <input
                    className="sheetNameInput"
                    placeholder="Sheet Name"
                    type="text"
                    value={selectedSheetName}
                    style={{fontSize:fontSize.medium,color:palette.primary.contrastText}}

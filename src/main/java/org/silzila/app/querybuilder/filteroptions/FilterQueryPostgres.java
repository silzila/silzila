...
public class FilterQueryPostgres {
    ...
    public static String getFilterOptions(ColumnFilter req, Table table) throws BadRequestException {
        System.out.println("=========== FilterQueryPostgres fn calling...");
        /*
         * ************************************************
        ...
        if (List.of("TEXT", "BOOLEAN").contains(req.getDataType().name()) ||
                (List.of("INTEGER", "DECIMAL").contains(req.getDataType().name())
                        && req.getFilterOption().name().equals("ALL_VALUES"))) {
            query = "SELECT DISTINCT " + req.getFieldName() + fromClause
                    + "ORDER BY 1";
        }
         ...
         */
                ...
                && req.getFilterOption().name().equals("MIN_MAX")) {

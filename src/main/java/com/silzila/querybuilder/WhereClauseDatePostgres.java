...
public class WhereClauseDatePostgres implements WhereClauseDate{
    ...
    public String buildWhereClauseDate(Filter filter) throws BadRequestException {
         ...
         */
            ...
            if (List.of("YEAR", "QUARTER", "MONTH", "DAYOFMONTH").contains(filter.getTimeGrain().name())) {
                field = "EXTRACT(" + timeGrainMap.get(filter.getTimeGrain().name()) + " FROM " + whereField + ")::INTEGER";
            } else if (filter.getTimeGrain().name().equals("DATE")) {
                field = "DATE(" + whereField + ")";
            }
        ...
        if(filter.getIsTillDate() && List.of("MONTH","DAYOFMONTH","YEARMONTH","YEAR","DAYOFWEEK","QUARTER","YEARQUARTER").contains(filter.getTimeGrain().name())){
            where = "(\n\t\t" + where+ TillDate.tillDate("postgresql", filter,whereField) + "\n\t\t)";
    ...
    public  String getDatePartExpression(String timeGrain, String columnName) {
        ...
        if (timeGrain.equals("YEAR")) {
            ...
            dateExpression = "CONCAT('Q', EXTRACT(QUARTER FROM " + columnName + ")::INTEGER)";
        } else if (timeGrain.equals("MONTH")) {
            dateExpression = "TRIM(TO_CHAR(" + columnName + ", 'Month'))";
        } else if (timeGrain.equals("YEARQUARTER")) {
            dateExpression = "TO_CHAR(" + columnName + ", 'YYYY') || '-Q' || TO_CHAR(" + columnName + ", 'Q'))";

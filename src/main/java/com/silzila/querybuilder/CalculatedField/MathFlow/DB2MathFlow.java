...
public class DB2MathFlow {

    private final static String vendor = "db2";
    ...
    public static void db2MathFlow(DataSchema dataschema,Flow flow,
        ...
        if (List.of("addition", "subtraction", "multiplication", "division").contains(flowType)) {
                ...
                processLogOperation(flow, fields, flowStringMap, flowKey, source, sourceType, calculatedFieldMap);
                    ...
                    CalculatedFieldProcessedSource
                                        .processAggregation(vendor,dataschema,flow,"", sourceType.get(0), source.get(0), fields, flowStringMap, flowKey,  false,calculatedFieldMap);
                }
    }

    // to process math basic operations - addition, subtraction, multiplicattion, division
    private static void processMathBasicOperations(DataSchema dataschema,Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
                                             List<String> result,String flowKey,List<String> source,List<String> sourceType,  Map<String,CalculatedFieldDTO> calculatedFieldMap) throws BadRequestException {
        

        for (int i = 0; i < source.size(); i++) {
            String processedSource = CalculatedFieldProcessedSource.getMathProcessedSource(source.get(i), sourceType.get(i), fields, flowStringMap, flow, i,flowKey, calculatedFieldMap);
        ...
        if(flow.getIsAggregation()){
            mathematicalExpression = CalculatedFieldProcessedSource
                                                    .processAggregation("db2",dataschema,flow, mathematicalExpression, "agg", null, fields, flowStringMap, flowKey,  true,calculatedFieldMap);
        }
    }

    // to process math single argument operations - absolute,ceiling,floor
    private static void processMathSingleArgumentOperations(DataSchema dataschema , Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
                                                         String flowKey,List<String> source, List<String> sourceType,  Map<String,CalculatedFieldDTO> calculatedFieldMap) throws BadRequestException {
        String processedSource = CalculatedFieldProcessedSource.getMathProcessedSource(source.get(0), sourceType.get(0), fields, flowStringMap, flow, 0,flowKey,calculatedFieldMap);
    ...
    // to process math multiple argument operations - minimum and maximum
    private static void processMultipleArgumentOperations(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
                                                         String flowKey,List<String> result, List<String> source, List<String> sourceType,  Map<String,CalculatedFieldDTO> calculatedFieldMap) throws BadRequestException {
        ...
        flowStringMap.put(flowKey, new FlowDTO(basicMathOperations.get(flow.getFlow()) + "(" + String.join(" ", result) + ")", "integer",flow.getIsAggregation()));
    }
    ...
    private static void processPowerOperation(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
        ...
        String processedSource = CalculatedFieldProcessedSource.getMathProcessedSource(source.get(0), sourceType.get(0), fields, flowStringMap, flow, 0,flowKey,calculatedFieldMap);
        flowStringMap.put(flowKey, new FlowDTO(basicMathOperations.get(flow.getFlow()) + "(" + processedSource + "," + source.get(1) + ")","integer",flow.getIsAggregation()));
    }

    // To process the log operation
    // 1st source -argument , 2nd source - base value
    private static void processLogOperation(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
                                          String flowKey,List<String> source, List<String> sourceType,  Map<String,CalculatedFieldDTO> calculatedFieldMap) throws BadRequestException {

        StringBuilder result = new StringBuilder();
        String processedSource = CalculatedFieldProcessedSource.getMathProcessedSource(source.get(0), sourceType.get(0), fields, flowStringMap, flow, 0,flowKey,calculatedFieldMap);
        String processedBase = CalculatedFieldProcessedSource.getMathProcessedSource(source.get(1), sourceType.get(1), fields, flowStringMap, flow, 1,flowKey,calculatedFieldMap);
        result.append("LN(").append(processedSource).append(")/LN(").append(processedBase).append(")");
        flowStringMap.put(flowKey, new FlowDTO(result.toString(),"integer",flow.getIsAggregation()));
    }

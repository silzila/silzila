...
public class DatabricksDateFlow {
    ...
    public static FlowDTO databricksDateFlow(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
    ...
    private static FlowDTO addDateInterval(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
        ...
        int intervalValue = Integer.parseInt(flow.getSource().get(1));
        String unit = flow.getSource().get(2).toLowerCase();

        String datePart = dateParts.get(unit);

        result.append(dateField).append(" + INTERVAL ").append(intervalValue).append(" ").append(datePart);
    ...
    private static FlowDTO calculateDateInterval(Flow flow, Map<String, Field> fields,
        ...
        String result = """
                    CASE
                        WHEN '%?' = 'day' THEN DATEDIFF(%!::DATE, %&::DATE)
                        WHEN '%?' = 'week' THEN DATEDIFF(%!::DATE, %&::DATE) / 7
                        WHEN '%?' = 'month' THEN MONTHS_BETWEEN(%!::DATE, %&::DATE)
                        WHEN '%?' = 'year' THEN FLOOR(MONTHS_BETWEEN(%!::DATE, %&::DATE) / 12)
                    END
                """;
        ...
        String aggregatedResult = !flow.getIsAggregation() ? result : aggregate(result, flow.getAggregation().get(0));
    ...
    private static FlowDTO getDatePartName(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
        ...
        switch (datePart) {
            case "month":
                result.append("DATE_FORMAT(").append(processedSource.get(0)).append(", 'MMMM')");
                break;
            case "day":
                result.append("DATE_FORMAT(").append(processedSource.get(0)).append(", 'EEEE')");
                break;
            default:
    ...
    private static FlowDTO getDatePartNumber(Flow flow, Map<String, Field> fields, Map<String, FlowDTO> flowStringMap,
        ...
        result.append(dateParts.get(flow.getSource().get(1))).append("(").append(processedSource.get(0)).append(")");
    ...
    private static FlowDTO getTruncateDateToPart(Flow flow, Map<String, Field> fields,
        ...
        result.append("DATE_TRUNC('").append(dateParts.get(flow.getSource().get(1))).append("',")

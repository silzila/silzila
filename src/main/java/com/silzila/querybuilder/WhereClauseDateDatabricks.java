...
public class WhereClauseDateDatabricks implements WhereClauseDate {
    ...
    public String buildWhereClauseDate(Filter filter) throws BadRequestException {
        ...
        String whereField = filter.getIsCalculatedField()
                || !"field".equals(filter.getConditionType().getLeftOperandType()) ? filter.getFieldName()
        ...
        if (filter.getIsTillDate()
                && List.of("MONTH", "DAYOFMONTH", "YEARMONTH", "YEAR", "DAYOFWEEK", "QUARTER", "YEARQUARTER")
                        .contains(filter.getTimeGrain().name())) {
            where = "(\n\t\t" + where + TillDate.tillDate("databricks", filter,whereField) + "\n\t\t)";
